...
static void processCommand(char* buffer, Print& out) {
  ...
  // FIX : verrou admin — les commandes de diagnostic (moteur, reboot, tare,
  // reglages thermiques) ne sont executees que si un badge NFC autorise a
  // ete scanne au prealable (voir admin.cpp). Toute autre commande reste
  // accessible a tous, par BT ou par port serie.
  if (isAdminCommand(buffer) && !isAdminUnlocked()) {
    out.println("E0AUTH - commande reservee a l'admin. Badgez le lecteur NFC pour deverrouiller.");
    return;
  }
  ...
  // -- ver ... --
    ...
    if (strcmp(argument, "arduino") == 0) {
      ...
      out.println("Drivers versions:");
      out.println(" - Admin handler: " + String(AUTH_HANDLER_VERSION));
      out.println(" - Battery driver: " + String(BATT_DRIVER_VERSION));
      out.println(" - Communication driver: " + String(COM_DRIVER_VERSION));
      out.println(" - ISR handler: " + String(ISR_HANDLER_VERSION));
      out.println(" - Traction driver: " + String(TRACTION_MODULE_VERSION));
      out.println(" - Turret driver: " + String(TOURELLE_DRIVER_VERSION));
      out.println(" - System module: " + String(SYSTEM_MODULE_VERSION));
  ...
  // - admin command (other) --
    ...
    if (strcmp(arg, "UID_AUTH show") == 0) {
      ...
      out.println("E0AUTH - adding new UID is not implemented in this version.");
    }
    else if (strcmp(arg, "UID_AUTH remove") == 0) {
      out.println("E0AUTH - removing UID is not implemented in this version.");
    }
    else {
      out.println("E0AUTH - unknown admin argument");
    }
  }
...
void updateCommunication(int swdRaw) {
  ...
  // 6. ENVOI DE LA TÉLÉMÉTRIE (uniquement si activée, uniquement par Bluetooth)
  if (isTelemetryActive && (currentMillis - lastTempSendTime >= telemetryIntervalMs)) {
