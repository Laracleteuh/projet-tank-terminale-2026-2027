...
static TickType_t lastRampTick = 0;
  ...
  switch (m.state) {
    ...
    case RUNNING:
      if (speed == 0) {
          ...
          setPot(m, 0);
          m.state = SWITCHING;
          m.switchStartTick = xTaskGetTickCount();
          relayWrite(m.pinFwd, false);
          relayWrite(m.pinBck, false);
      } else {
    ...
    case SWITCHING:
      if ((xTaskGetTickCount() - m.switchStartTick) * portTICK_PERIOD_MS >= 200) {
          m.currentDir = targetDir;
          m.state = RUNNING;
      }
      break;
...
void updateTraction(int speed, int steering, int vrbRaw) {
  if (diagMode) {
    if (diagSide == 'L') {
      ...
      updateMotor(rightMotor, 0); 
    } else if (diagSide == 'R') {
      updateMotor(leftMotor, 0);
      updateMotor(rightMotor, diagSpeed); 
    }
    return; 
  }

  TickType_t currentTick = xTaskGetTickCount();
  if (lastRampTick == 0) lastRampTick = currentTick;
  unsigned long dt = (currentTick - lastRampTick) * portTICK_PERIOD_MS;
  lastRampTick = currentTick;

  if (dt > MAX_RAMP_DT_MS) dt = MAX_RAMP_DT_MS;

  int safeVrb = constrain(vrbRaw, 1000, 2000);
  float rampingFactor = map(safeVrb, 1000, 2000, 1, 100) / 100.0f;
