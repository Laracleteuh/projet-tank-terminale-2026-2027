...
static volatile TickType_t lastRcSignalTick = 0;
...
void isr_y() {
  if (digitalRead(rightJoystickY) == HIGH) timer_y = micros();
    ...
    lastRcSignalTick = xTaskGetTickCountFromISR();
  }
}
...
// -- Failsafe -- 
bool checkFailsafe() {
  TickType_t lastSignal;
  taskENTER_CRITICAL();
  lastSignal = lastRcSignalTick;
  taskEXIT_CRITICAL();
  
  return ((xTaskGetTickCount() - lastSignal) * portTICK_PERIOD_MS > 200);
}
